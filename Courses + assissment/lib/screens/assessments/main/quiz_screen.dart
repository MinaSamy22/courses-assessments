...
class _QuizScreenState extends State<QuizScreen> with WidgetsBindingObserver {
  ...
  void _handleTimeOut() {
    ...
    // Removed _markQuizCompleted() here - only mark when actually completed
  ...
  Future<void> _loadQuizQuestions() async {
    try {
      ...
      final quizData = data.firstWhere(
        ...
        orElse: () => throw Exception('Assessment "${widget.quizTitle}" not found'),
      );
      ...
      setState(() {
        errorMessage = 'Error loading Assessment: ${e.toString()}';
        isLoading = false;
      });
    }
  }
  ...
  void moveToNextQuestion() {
    ...
    if (currentQuestionIndex < questions.length - 1) {
      ...
      _quizTimer.dispose();
      _markQuizCompleted(); // Only mark as completed when all questions are answered
